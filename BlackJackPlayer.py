    ...
    for _ in xrange(iterations):
        for state in value_table:
            ...
            if Y < 21:
                ...
                Q_state_hit = sum(n[2] + n[1] * value_table[n[0]] for n in hit_neighbors)

            # Split action
            if P:
                split_neighbors = dP[state]
                if X == 2 and Y == 12:
                    Q_state_split = 2 * sum(n[1] * Qfunction(n[0], 'TS', prob) for n in split_neighbors)
                else:
                    Q_state_split = 2 * sum(n[2] + n[1] * value_table[n[0]] for n in split_neighbors)
